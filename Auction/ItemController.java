...
public class ItemController extends Thread{
	...
	public void run(){
		...
		auctionSleep(5000);
		broadcast("The auction starts in 5 seconds.Waiting...");
		auctionSleep(5000);
		broadcast("The auction starts:"); 
		for (Item it : items)
		{	//initialize the variables for the auction of a single item
			item_id=it.getId();
			description=it.getDescription();
			startingPrice=it.getStartingPrice();
			auctioneer.interestedUsers.clear();
			auctioneer.time_interval=auctioneer.interval_L;
			int counter=5;
			String itemHolder="no_holder";
			areYouInterestedFlag=true;
			//
